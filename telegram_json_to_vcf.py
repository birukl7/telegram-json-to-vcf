...
def vcf(fname, lname, cell):
    cell = cell.replace(" ", "")  # Remove spaces from the phone number

    # Ensure correct formatting for Ethiopian numbers
    if cell.startswith("00251"):
        # Replace "00251" with "+251"
        formatted_cell = "+251" + cell[5:]
    elif cell.startswith("0"):
        # Replace the leading "0" with "+251"
        formatted_cell = "+251" + cell[1:]
    elif cell.startswith("+251"):
        # Keep the number as-is for +251 format
        formatted_cell = cell
    else:
        # Use the number as-is for non-Ethiopian formats
        formatted_cell = cell
    ...
    vcard = (
        ...
        f"N:{lname};{fname};;;\n"
        f"TEL;TYPE=CELL:{formatted_cell}\n"
        "END:VCARD\n"
    )
